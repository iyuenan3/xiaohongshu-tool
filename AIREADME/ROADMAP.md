...
## Now（当前焦点）
- **license server 迁 alicloud-bj — 服务已建成、入部署阶段** (D9 **B'** token-only + Hono Node 服务，代码落 monorepo `doubleL-license`/`apps/xhs-license`，ADR-011/012/013)：服务端全实现 (provision / suspend / quota + node-cron 月度重置 + admin-ui) + 全生命周期 e2e 对真 newapi 通过 (3 commits 未 push)。**剩部署** = 轮换签名密钥 + 上 bj Docker + 转达 newapi-proxy (Caddy path 路由 / tools 限速 / auto-llm 成本) + 客户端 (cert 信任 IP + 端点 + 发新版) + **砍 CF**(全落 bj)。前置 newapi-proxy M1 已部分就绪 (专用账号 `xiaohongshu-tool`(id=4) + 访问令牌已拿 + B' 全链路实测通过)。
