...
- **v0.8.1 已发版** (2026-05-26)：GH Release arm64 mac dmg/zip + win exe (Intel x64 runner 排队卡 13h 弃→后补) + bj `latest_version=0.8.1` 已设 (老客户端启动即提示升级)。含改进 → feed **seq 序号寻址**根治 LLM 抄错 24 位长 id 卡 60s (互动工具只收 seq、Go `feed_registry` 查真实 id、兼容 feed_id 给 workflow) + 评论 prompt 短而真 + like 免二次确认 + workflow 拆 **定时点赞/定时评论** (删合并版、清表孤儿) + dmg 指引精简+终端 symlink + 独立窗口 xhsVisible race 修 + E2E 全套适配 bj/独立窗口/CDP 动态发现 (11 套件 0 fail)；**发布卡死修复** (Windows 内测旧 beta 日志暴露: attach 选中用户停留页 → 跨子域 Navigate 假成功 → upload-content 在 300s 下死磕 5 分钟; 修 = upload-content 20s 快速失败 + Navigate 重试 3 次 + renderer callApi 240s 兜底) + updater 清 electron-updater 死依赖 (404 是旧 beta 遗留、当前 /version 早替代)。详见 memory `project_m7_workflow` / `feedback_pitfalls` 坑 37·38·39 / `reference_e2e_test` 2026-05-26 节。
